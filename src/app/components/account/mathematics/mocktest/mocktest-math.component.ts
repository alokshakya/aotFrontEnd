...
export class MocktestMathComponent implements OnInit {
    ...
    setSofPattern(){
        this.patternArray = this.subjectInfo.patternObject['Mathematics'];
    }

    setMockTest(){
        if(this.test.math.hasOwnProperty('mock_test')){
            var completed = this.test.math['mock_test']['total_completed'];
